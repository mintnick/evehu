...
module.exports = async function (app) {
    if (m == false) {
        ...
        if (parseInt(await app.redis.scard('evehu:affiliates')) == 0) {
            try {
                let chars = await app.mysql.query('select character_id from characters where last_update < (NOW() - INTERVAL 1 DAY) and corporation_id != 1000001 order by last_update limit 100000');
